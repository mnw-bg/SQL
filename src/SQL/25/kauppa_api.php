...
function getAllAutot() {
    global $conn;
    $stmt = $conn->prepare("SELECT ID, Nimi, Hinta, Kuvaus, katekoria FROM Tuote");
    $stmt->execute();
...
function getAuto($id) {
    global $conn;
    $stmt = $conn->prepare("SELECT ID, Nimi, Hinta, Kuvaus, katekoria FROM Tuote WHERE ID = ?");
...
function addAuto() {
    ...
    if (!empty($data['Nimi']) && !empty($data['Hinta']) && !empty($data['Kuvaus'])) {
        $Nimi = $data['Nimi'];
        $Hinta = $data['Hinta'];
        $Kuvaus = $data['Kuvaus'];
        $katekoria = $data['katekoria'];

        $stmt = $conn->prepare("INSERT INTO Tuote (Nimi, Hinta, Kuvaus, katekoria) VALUES (?, ?, ?, ?)");
        $stmt->bind_param("ssss", $Nimi, $Hinta, $Kuvaus, $katekoria);
...
function updateAuto($data) {
    global $conn;
    if (!empty($data['ID']) && !empty($data['Nimi']) && !empty($data['Hinta']) && !empty($data['Kuvaus'])) {
        $id = intval($data['ID']);
        $Nimi = $data['Nimi'];
        $Hinta = $data['Hinta'];
        $Kuvaus = $data['Kuvaus'];
        ...
        $stmt = $conn->prepare("UPDATE Tuote SET Nimi = ?, Hinta = ?, Kuvaus = ?, katekoria = ? WHERE ID = ?");
        $stmt->bind_param("sssss", $Nimi, $Hinta, $Kuvaus, $katekoria, $id);
...
function deleteAuto($id) {
    global $conn;
    $stmt = $conn->prepare("DELETE FROM Tuote WHERE ID = ?");
    $stmt->bind_param("i", $id);
